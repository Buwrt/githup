...
Android 上的第三方 GitHub 客户端。用 WebView 承载一套纯前端单页应用，原生层只负责网络、文件、下载安装等 Web 做不了的事 —— APK 只有 **244 KB** 左右，却覆盖了浏览仓库、看 Issue / PR、查 Actions、发 Release、上传文件，甚至云端打包 APK 这些事。
...
| 变化位 | 例子 | 行为 |
|---|---|---|
| 第一位 x（大版本） | `1.1.2` → `2.0.0` | **强制更新**。弹层不可关闭：没有关闭按钮、点遮罩无效、返回键也无法绕过，必须更新才能继续用 |
| 第二位 y（功能） | `1.1.2` → `1.2.0` | 可选更新。弹窗给「立即更新 / 稍后提醒 / 跳过此版」 |
| 第三位 z（修复） | `1.1.2` → `1.1.3` | 可选更新，同上 |

**什么时候会打扰你**，分两条：

| 时机 | 有更新 | 已是最新 | 检查失败 |
|---|---|---|---|
| 打开软件 | 弹更新窗（大版本强制） | **完全静默，不弹任何东西** | 静默 |
| 手动点「检查更新」 | 弹更新窗 | 明确提示「**您已是最新版本**」+ 当前版本号 | 提示原因 |

其它规则：

- **打开即检查**：App 启动立刻发起检测，不再等几秒
- **6 小时节流**：6 小时内不重复请求，避免频繁打扰
- **跳过只对该版本生效**：记下被跳过的版本，但大版本永远会拦（强制更新不允许跳过）
- **版本号读不到时静默**：取不到 `x.y.z` 一律判为「无法比较」，**绝不编造 `0.0.0`** —— 假版本号会被当成「从 0.0.0 大版本升级」而误报强制更新（1.1.1 版本踩过这个坑，已在 1.1.2 修掉）
